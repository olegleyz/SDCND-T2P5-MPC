 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t=0; t < N; t++) {
      ...
      fg[0] += 1500 * pow(vars[cte_start + t] - ref_cte, 2);
      fg[0] += 1500 * pow(vars[epsi_start + t] - ref_epsi, 2);
      fg[0] += pow(vars[v_start + t] - ref_v, 2);
      if (t < N-1) {
        fg[0] += 20 * pow(vars[delta_start + t], 2);
        fg[0] += 40 * pow(vars[a_start + t], 2);
      }
      if (t < N - 2) {
        fg[0] += 200 * pow(vars[delta_start + 1 + t] - vars[delta_start + t], 2);
        fg[0] += 100 * pow(vars[a_start + 1 + t] - vars[a_start + t], 2);
      }
    }
